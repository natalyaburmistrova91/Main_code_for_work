    ...
    if ready_answer == 'N':
        ...
        for i in sorted_set_of_employees:
            ...
            if employees_info is None:
                ...
                if employees_info.get('Login в системе') is not None:
                    LIST_OF_EMAILS.append(employees_info['Login в системе'])
                else:
                    LIST_WITHOUT_LOGIN.append(employees_info['ФИО'])
        f_6 = open(f'{WAY}\list_of_emails.txt', 'w', encoding="utf-8")
        if len(UNEXPECTED_LIST) != 0:
            f_6.write(f'Не нашел в списке 1С:\n')
            for el in UNEXPECTED_LIST:
                f_6.write(f'{el}\n')
        if len(LIST_WITHOUT_LOGIN) != 0:
            f_6.write(f'Нет логина в базе:\n')
            for el in LIST_WITHOUT_LOGIN:
                f_6.write(f'{el}\n')
